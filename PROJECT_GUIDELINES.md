# 项目开发规范与架构指南

本项目采用 **Feature-First (功能优先)** 的架构模式，旨在确保代码在业务增长时保持高内聚、低耦合。

## 1. 目录结构规范

### `src/app` (路由层) - 基于路由组架构

#### 1.1 路由组 (Route Groups) 概述

采用 **Route Groups** 模式，用 `(groupName)` 命名规范组织相关路由。括号不影响 URL 路径，但提供逻辑分组和共享 Layout：

```
src/app/
├── (auth)/              # 认证路由组
│   ├── layout.tsx       # 认证流程共享布局（居中表单）
│   ├── login/
│   │   └── page.tsx
│   ├── register/
│   │   └── page.tsx
│   ├── forgot-password/
│   │   └── page.tsx
│   └── reset-password/
│       └── page.tsx
│
├── (public)/            # 公开内容路由组
│   ├── layout.tsx       # 继承全局 Navbar/Footer
│   ├── page.tsx         # 主页
│   ├── blog/
│   │   └── page.tsx
│   ├── market/
│   │   └── page.tsx
│   ├── pricing/
│   │   └── page.tsx
│   └── detail/
│       └── [id]/
│           └── page.tsx
│
├── (protected)/         # 受保护的仪表板路由组
│   ├── layout.tsx       # 认证守卫 + 角色检查
│   ├── account/
│   │   └── page.tsx
│   └── console/
│       ├── layout.tsx   # 控制台特定布局
│       ├── page.tsx     # 仪表板主页
│       └── settings/
│           ├── layout.tsx
│           └── page.tsx
│
├── _demo/               # 开发演示路由组（下划线前缀不生成路由）
│   ├── layout.tsx
│   └── typography/
│       └── page.tsx
│
├── api/                 # API 路由
│   ├── auth/
│   └── user/
│
├── layout.tsx           # 全局根布局（Navbar, Footer, Providers）
└── globals.css
```

#### 1.2 各路由组的职责与布局策略

| 路由组 | 用途 | 布局特点 | 访问控制 |
|------|------|--------|--------|
| **(auth)** | 登录、注册、密码重置 | 居中表单布局，最大宽度 28rem | 无需认证 |
| **(public)** | 营销内容、博客、产品详情 | 继承全局 Navbar/Footer | 无需认证 |
| **(protected)** | 用户仪表板、设置、账户管理 | 完整布局 + 侧边栏/菜单 | **需要认证** |
| **_demo** | 组件展示、开发工具 | 开发环境专用 | 内部使用 |
| **api** | RESTful 端点 | 无布局（路由处理函数）| 根据端点定义 |

#### 1.3 根布局与 Providers

**`src/app/layout.tsx`** 是全局父布局，负责：
1. 设置全局样式、字体、元数据
2. 配置 ThemeProvider（主题切换）
3. 配置 AuthProvider（认证上下文）
4. 渲染全局 Navbar 和 Footer（始终可见）
5. 通过 `{children}` 嵌入子布局和页面

所有路由组的 Layout 应继承全局 Navbar/Footer，避免重复渲染。

#### 1.4 页面文件规范

- **职责**: 定义路由、布局 (Layout)、元数据 (Metadata) 和服务端数据获取。
- **规范**: 页面组件应保持"薄"，主要负责组合 `features` 中的业务组件。避免在页面文件中直接定义复杂的表单或业务逻辑。
- **每个 page.tsx 应包含**:
  - 静态或动态元数据配置
  - 数据获取逻辑（如需要）
  - 组合相关的 feature 组件

### `src/features` (业务功能层)
- **职责**: 包含特定业务领域的所有逻辑，与路由组独立，基于功能域组织。
- **特点**: 一个 feature 通常会被多个页面或路由组使用，实现代码复用。
- **内部结构**:
  - `actions/`: Server Actions，负责处理表单提交和业务逻辑。
  - `components/`: 该功能特有的 UI 组件。
  - `hooks/`: (可选) 该功能特有的 React Hooks。
  - `schemas.ts`: Zod 校验模式。
  - `types.ts`: TypeScript 类型定义。
  - `index.ts`: 公共 API 导出文件。**外部模块只能通过 index.ts 导入该 feature 的内容。**

**示例**: `src/features/auth/` 包含认证相关的所有逻辑（表单、Server Actions、类型、数据验证），被 `/(auth)/login`、`/(auth)/register` 等多个页面共同使用。

### `src/components` (通用组件层)
- `ui/`: 基础原子组件 (如 Button, Input)，通常由 shadcn/ui 生成。
- `shared/`: 跨业务功能的通用组件 (如 Navbar, Footer, ThemeToggle)。

### `src/lib` (基础设施层)
- `db/`: 数据库配置、Schema 定义。
- `db/dao/`: Data Access Objects，封装底层的数据库 CRUD 操作。
- `services/`: 复杂的跨模块业务逻辑。
- `utils.ts`: 通用工具函数。

## 2. 开发原则

1.  **单向依赖**: `features` 之间应尽量减少直接依赖。如果多个 feature 需要共享逻辑，考虑将其提取到 `lib` 或 `components/shared`。
2.  **DAO 封装**: 不要直接在 Server Actions 中调用 Drizzle 的 `db.select()` 等方法，应通过 `lib/db/dao` 进行封装，便于测试和复用。
3.  **Server Actions**: 所有的表单提交和状态变更应使用 Server Actions。
4.  **类型安全**: 充分利用 TypeScript 和 Zod，确保从数据库到 UI 的全链路类型安全。
5.  **导出规范**: 始终使用 `index.ts` 导出 feature 的公共接口，避免深层导入（如 `import { ... } from "@/features/auth/actions/login"`）。
6.  **路由组与 Feature 的关系**: 
   - **路由组** (`(auth)`, `(public)`, `(protected)`) 是 **导航结构** 和 **布局层级**，定义页面访问流程和共享 UI。
   - **Features** 是 **业务逻辑域**，与路由组解耦，可被多个路由重用。
   - 示例: `/(auth)/login` 页面导入 `@/features/auth` 中的登录表单组件和 `loginAction`。
7.  **Layout 最小化原则**:
   - 根 Layout 提供全局 Navbar、Footer、Providers（主题、认证）。
   - 路由组 Layout 仅添加该组特定的布局需求（如 `(auth)` 的居中容器）。
   - 避免在每个 Layout 中重复渲染 Navbar/Footer，使用从父 Layout 继承。

## 3. 命名约定

-   **组件**: 大驼峰 (PascalCase)，如 `LoginForm.tsx`。
-   **文件/文件夹**: 小写横杠 (kebab-case)，如 `update-profile.ts`。
-   **Actions**: 以 `Action` 结尾，如 `loginAction`。
-   **DAO**: 以 `dao` 结尾，如 `usersDao`。
-   **路由组**: 用 `(groupName)` 格式，如 `(auth)`, `(public)`, `(protected)`。

## 3.1 路由组最佳实践 (Route Groups Best Practices)

### 认证路由组 `(auth)`
- **用途**: 所有认证流程（登录、注册、密码重置）
- **URL 示例**: `/login`, `/register`, `/forgot-password`, `/reset-password`
- **Layout 特点**: 
  ```tsx
  // /(auth)/layout.tsx - 居中表单布局
  export default function AuthLayout({ children }) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="w-full max-w-md">{children}</div>
      </div>
    );
  }
  ```
- **不应包含**: Navbar、Footer、侧边栏等（保持极简）
- **元数据**: 使用 `noindex, nofollow` 防止搜索引擎索引
- **访问控制**: 无需认证即可访问

### 公开路由组 `(public)`
- **用途**: 所有营销内容、公开博客、产品信息、定价页面
- **URL 示例**: `/`, `/blog`, `/market`, `/pricing`, `/detail/[id]`
- **Layout 特点**: Layout 为空或仅返回 `{children}`（继承全局 Navbar/Footer）
  ```tsx
  // /(public)/layout.tsx - 继承全局布局
  export default function PublicLayout({ children }) {
    return <>{children}</>;
  }
  ```
- **元数据**: 包含完整的 SEO 元数据（title, description, keywords）
- **每个 page.tsx**: 应设置页面级别的 `generateMetadata()`
- **访问控制**: 无需认证即可访问

### 受保护路由组 `(protected)`
- **用途**: 所有需要认证的用户功能（仪表板、设置、账户）
- **URL 示例**: `/console`, `/console/settings`, `/account`
- **Layout 必需**: 实现认证守卫
  ```tsx
  // /(protected)/layout.tsx - 认证守卫
  'use client';
  import { useAuth } from '@/features/auth';
  import { redirect } from 'next/navigation';
  
  export default function ProtectedLayout({ children }) {
    const { user } = useAuth();
    
    if (!user) {
      redirect('/login');
    }
    
    return <>{children}</>;
  }
  ```
- **特点**: 未认证用户自动重定向到登录页
- **子布局**: 如 `console/layout.tsx` 可添加仪表板特定的侧边栏/菜单
- **访问控制**: **需要认证**

### 演示路由组 `_demo`
- **用途**: 开发期间的演示页面、组件展示、排版演示
- **URL**: 下划线前缀 `_demo` 不会生成路由，但可通过 `/_demo/typography` 访问
- **特点**: 仅在开发中使用，生产环境应移除
- **访问方式**: `http://localhost:3000/_demo/typography`

### API 路由
- **结构**: `/api/auth/`, `/api/user/` 等
- **文件**: 使用 `route.ts` 定义 GET、POST 等处理函数
- **返回格式**: 统一返回 JSON
- **示例**:
  ```typescript
  // src/app/api/auth/route.ts
  export async function POST(request: Request) {
    // 处理认证请求
    return Response.json({ data: {...} });
  }
  ```

## 5. 字体与排版规范

### 5.1 字体族配置 (Font Family)

采用分层回退机制，优先使用系统原生字体，避免外部字体加载带来的性能问题。

**无衬线字体 (Sans-serif) - 用于标题、正文、UI 元素**
- **iOS/macOS**: San Francisco, PingFang SC
- **Windows**: Segoe UI, Microsoft YaHei  
- **Android**: Roboto, Noto Sans CJK SC
- **回退链**: `-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei", "Simplified Chinese", sans-serif`

**等宽字体 (Monospace) - 用于代码、表单数据、数值**
- 回退链: `ui-monospace, SFMono-Regular, "SF Mono", Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace`

### 5.2 字体渲染优化 (Rendering)

在 `body` 标签中应用以下 CSS 优化，修复不同浏览器下的字体粗细不均和锯齿问题：

```css
body {
  -webkit-font-smoothing: antialiased;  /* Mac 系统平滑 */
  -moz-osx-font-smoothing: grayscale;   /* Firefox 平滑 */
  text-rendering: optimizeLegibility;   /* 文本微调 */
  -webkit-text-size-adjust: 100%;       /* 禁用移动端自动放大 */
}
```

### 5.3 字阶与行高规范 (Typography Scale)

采用 4px 为基数的倍数关系，确保视觉节奏严谨。所有样式通过 Tailwind CSS 工具类应用。

| 等级 | 字号 | 行高 | 权重 | Tailwind 类 | 使用场景 |
|-----|------|------|------|------------|----------|
| **一级标题** | 32px | 1.2 | 600 | `text-3xl font-semibold leading-tight` | 页面主标题 |
| **二级标题** | 24px | 1.3 | 600 | `text-2xl font-semibold leading-snug` | 模块标题 |
| **三级标题** | 20px | 1.4 | 600 | `text-xl font-semibold` | 小组标题 |
| **强调正文** | 16px | 1.5 | 500 | `text-base font-medium leading-relaxed` | 卡片内容 |
| **标准正文** | 14px | 1.5 | 400 | `text-sm leading-relaxed` | 默认文本 |
| **辅助文字** | 12px | 1.4 | 400 | `text-xs leading-normal` | 表单提示、页脚 |

**重要 🔴**: 中文环境建议使用 `font-semibold (600)` 而非 `font-bold (700)`，以防文字发虚。

### 5.4 颜色与对比度 (Color & Contrast)

严禁使用纯黑色，采用灰度梯队以减轻视觉疲劳：

- **一级文本** (标题/正文): `text-foreground` - `rgba(0, 0, 0, 0.88)` (亮色) / `rgba(255, 255, 255, 0.92)` (暗色)
- **二级文本** (次要信息): `text-muted-foreground` - `rgba(0, 0, 0, 0.65)` (亮色) / `rgba(255, 255, 255, 0.65)` (暗色)  
- **三级文本** (禁用/占位符): `text-muted` - 更淡的灰度

### 5.5 性能与加载 (Performance)

- ✅ 优先使用系统字体，避免外部 WebFont 加载延迟
- ✅ 若必须使用自定义字体，仅允许 `.woff2` 格式
- ✅ 添加 `font-display: swap;` 避免 FOIT (文字闪烁)
- ✅ 中文字体必须子集化，体积 ≤ 200KB
- ✅ 推荐使用 Google Fonts 或 Alibaba 免费方案

### 5.6 实施检查清单 (Checklist)

- [ ] 是否在 Mac 与 Windows 下检查过字体粗细？
- [ ] 行高是否严格按表格执行？  
- [ ] 所有数值/代码是否使用 `font-mono`？
- [ ] 链接/按钮点击态是否保持字体颜色一致性？
- [ ] 移动端是否测试过字体缩放？

## 6. H5 兼容性与移动端规范

### 6.1 视口配置 (Viewport)

所有页面必须在 `<head>` 中包含正确的视口配置，通过 Next.js `metadata` API 设置。

```typescript
export const metadata: Metadata = {
  viewport: {
    width: "device-width",
    initialScale: 1,
    viewportFit: "cover",
    userScalable: false,
  },
  appleWebApp: {
    capable: true,
    statusBarStyle: "black-translucent",
  },
};
```

### 6.2 触摸与交互规范 (Touch & Interaction)

| 规则 | 标准 | 实现方案 |
|-----|------|--------|
| **按钮最小点击区域** | 44×44px | 使用 `touch-target` 或 `h-11 w-11` 或更大 |
| **按钮间距** | ≥8px | 使用 `gap-2` 或 `gap-3` |
| **长按反馈** | 需要视觉反馈 | 使用 `active:scale-[0.98]` |
| **双击缩放禁用** | `user-scalable=no` | 已在 viewport 配置中 |
| **链接点击区域扩展** | 周围 8px | 使用 `px-3 py-2.5` 或 `touch-target` |
| **禁用长按菜单** | 移除原生菜单 | 使用 `-webkit-touch-callout: none` |

### 6.3 字体与输入框优化 (Mobile Typography)

**关键原则**:
- 所有输入框基础字号 ≥ 16px，防止 iOS Safari 自动放大
- 禁用输入框的原生样式 (`-webkit-appearance: none`)
- 移除 iOS tap highlight

```css
/* 输入框防止自动放大 */
input, textarea, select {
  font-size: 16px; /* >= 16px */
  -webkit-appearance: none;
  -moz-appearance: none;
  appearance: none;
}

/* 移动端响应式字号 */
@media (max-width: 640px) {
  h1 { @apply text-2xl; }    /* 32px → 24px */
  h2 { @apply text-xl; }     /* 24px → 20px */
  h3 { @apply text-lg; }     /* 20px → 18px */
}
```

### 6.4 安全区域适配 (iOS Notch & Home Indicator)

使用 CSS 环境变量适配 iOS 刘海屏和 Home Indicator：

```css
.safe-top { @apply pt-[env(safe-area-inset-top)]; }
.safe-bottom { @apply pb-[env(safe-area-inset-bottom)]; }
.safe-left { @apply pl-[env(safe-area-inset-left)]; }
.safe-right { @apply pr-[env(safe-area-inset-right)]; }
```

应用方式：
- 导航栏顶部: `<header className="safe-top">...</header>`
- 底部导航: `<footer className="safe-bottom">...</footer>`

### 6.5 性能优化 (Mobile Performance)

- ✅ **图片**: 使用 Next.js `<Image />` 组件，启用 `priority` 和 `placeholder`
- ✅ **CSS**: Tailwind JIT 编译，产生最小 CSS 体积
- ✅ **重排**: 避免动态高度变化，使用固定尺寸
- ✅ **懒加载**: 使用 `loading="lazy"` 延迟加载图片
- ✅ **防抖节流**: 表单提交、滚动事件使用防抖/节流

### 6.6 响应式设计断点 (Responsive Breakpoints)

| 设备 | Tailwind 前缀 | CSS 范围 | 应用场景 |
|-----|-------------|---------|--------|
| **手机竖屏** | None | < 640px | 单列布局，堆叠组件 |
| **手机横屏** | `sm` | ≥ 640px | 两列布局，紧凑间距 |
| **平板** | `md` | ≥ 768px | 三列布局，正常间距 |
| **桌面** | `lg`/`xl` | ≥ 1024px | 多列布局，大间距 |

### 6.7 H5 检查清单 (Checklist)

- [ ] 所有按钮/链接 ≥ 44×44px，或有充分的 padding
- [ ] 输入框字号 ≥ 16px（防止 iOS 自动放大）
- [ ] 内容宽度使用 `max-w-5xl` 限制，防止过度拉伸
- [ ] 顶部导航栏预留安全区（`safe-top`）
- [ ] 底部操作栏预留安全区（`safe-bottom`）
- [ ] 表单有明显的 `:focus` 和 `:active` 反馈
- [ ] 图片使用 Next.js `<Image />` 并启用响应式
- [ ] 触摸目标间距 ≥ 8px
- [ ] 在真机上测试竖屏和横屏模式
- [ ] 在 2G/3G 网络下测试加载速度

